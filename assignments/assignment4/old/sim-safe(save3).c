...
//victim cache =================================================================================================================
//victim cache =================================================================================================================
void cvictim_cache_access( struct cache *c, unsigned addr,counter_t *load_miss,counter_t *store_miss, enum md_opcode op)
{
unsigned  index, tag;
index = (addr>>c->m_set_shift)&c->m_set_mask;
tag = (addr>>c->m_tag_shift);
...
if(change) {
  assert (save < pow(2,c->m_associativity));
  cvictim_cache_access(victim, addr, load_miss, store_miss, op);
  c->m_tag_array[index+save*assertion].m_valid = 1;
  c->m_tag_array[index+save*assertion].m_tag = tag;
  c->m_tag_array[index+save*assertion].LRU = sim_num_insn;
  c->m_tag_array[index+save*assertion].m_modified = mod;
...
sim_main(void)
{
  ...
  int bassociativity_bits = log2(1); //8-way has 3 bits which is log2(8)
