...
#include "machine.def"
    ...
    */
    if (!(MD_OP_FLAGS(pI->op) & F_MEM)){
        // check for RAW hazard
        for ( i=0; i < 3; ++i ) { // for each potential source operand...
            if ( pI->src[i] != NULL ) {// src[i] has not written to register file this cycle or earlier
                // sim_cycle+1 says that src[i] will read from the WB stage            
                if( pI->src[i]->donecycle > sim_cycle+1) {
                    if(MD_OP_FLAGS(pI->src[i]->op) & F_LOAD){
                        //if (pI->src[i]->status == EXECUTED){                        
                                //Execute will always be finished before the execute stage of the next command so only need to check if the
                                //register is being changed in the MEM stage and is a load
                                pI->stalled = 1;
                                return;
                            //}
                        }
    ...
    if (MD_OP_FLAGS(pI->op) & F_STORE){
        ...
        for ( i=0; i < 3; ++i ) { // for each potential source operand...
            if ( pI->src[i] != NULL ) {    
                ...
                if( pI->src[i]->donecycle > sim_cycle+1) {
                    // src[i] has not written to register file this cycle or earlier
                    if (pI->src[i]->status == MEMORY_STAGE_COMPLETED){
                        if(MD_OP_FLAGS(pI->src[i]->op)&F_LOAD){
                            //if (!(MD_OP_FLAGS(pI->src[i]->op)&F_LOAD)))
                                //Execute will always be finished before the execute stage of the next command so only need to check if the
                                //register is being changed in the MEM stage and is a load
