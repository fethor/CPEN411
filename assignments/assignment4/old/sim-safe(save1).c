...
stat_reg_formula(sdb, "sim_bcache_store_miss_rate",
  ...
  "100*(sim_num_bcache_miss_load / g_bnum_stores)", NULL);
...
if(change) {
  ...
  if (c->m_tag_array[index*(save+1)].m_modified){
    *wb_event = *wb_event +1;
  }
  c->m_tag_array[index*(save+1)].m_valid = 1;
  c->m_tag_array[index*(save+1)].m_tag = tag;
...
sim_main(void)
{
  ...
  int btotal_blocks = (32*pow(2,10))/64; //32KB / 32B = 1024 blocks
  int bassociativity_bits = log2(8); //4-way has 2 bits which is log2(4)
  ...
  bcache->m_tag_array = (struct block *) calloc( sizeof(struct block), btotal_blocks );
  //size of the index, block bits, everything else is for tag
