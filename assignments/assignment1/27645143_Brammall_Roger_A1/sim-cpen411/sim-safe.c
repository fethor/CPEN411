...
/*Save the registers at the beginning of the command to compare later*/
	unsigned int RSsave = GPR(RS);
	unsigned int RTsave = GPR(RT);
	unsigned int RDsave = GPR(RD);
...
*/
	...
	unsigned int RSnew = GPR(RS);
	unsigned int RDnew = GPR(RD);
	unsigned int RTnew = GPR(RT);
