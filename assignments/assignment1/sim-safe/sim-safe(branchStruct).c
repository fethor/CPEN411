...
static struct BranchDistance* Zero;
...
void printOutput(void) {
	...
	struct BranchDistance* readPointer = malloc(sizeof(Zero));
	//printf("HEllo\n");
	//int hi = Zero->distance;
	//readPointer = Zero;
	//printf("Offset: %d ", readPointer->distance);
	/*
...
/*intialize struct and make distance and pointer NULL to start*/
/* notably a bit slow due to always checking the Zero struct but can't know for sure an offset that is guaranteed to be used */
...
#include "machine.def"
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
	{
      ...
      if( MD_OP_FLAGS(op) & F_COND ) {
		...
		/*Checks the pointer to see if pointing to the end of the structs or if pointing at zero to decide how long to check*/

		while ((nextPointer->pointer != NULL) || (&nextPointer == &Zero)){
			nextPointer = nextPointer->pointer;

			/* adds to count if IMM (branch offset) is equal to the distance value in the struct */
			if (nextPointer->distance == IMM){
				nextPointer->count = nextPointer->count + 1;
				break;
			}
			else {
				/* Checks if at the end of the pointers and make a new struct or move to the next struct and run while again */
