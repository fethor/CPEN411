...
//part d
static counter_t g_dcache_miss;
static counter_t g_dcache_saved;
static counter_t g_dcache_prefetch;
...
//part c
  ...
  //part d
stat_reg_counter(sdb, "sim_num_dcache_miss",
  "total number of instruction dcache miss",
  &g_dcache_miss, 0, NULL);


stat_reg_counter(sdb, "sim_num_dcache_prefetch",
  "total number of instruction dcache prefetch",
  &g_dcache_prefetch, 0, NULL);

stat_reg_counter(sdb, "sim_num_dcache_save",
  "total number of instruction dcache save",
  &g_dcache_saved, 0, NULL);


stat_reg_formula(sdb, "sim_dcache_miss_rate",
  "instruction dcache load miss rate (percentage)",
  "100*(sim_num_dcache_miss / sim_num_insn)", NULL);


stat_reg_formula(sdb, "sim_dcache_save_rate",
  "instruction dcache prefetch usage (percentage)",
  "100*(sim_num_dcache_save / sim_num_dcache_prefetch)", NULL);
...
//part d =================================================================================================================
//prefetch
void dprefetch_cache_access( struct cache *c, unsigned addr, counter_t *prefetch )
{
unsigned  index, tag;
...
c->m_tag_array[index+save*assertion].LRU = 0;
c->m_tag_array[index+save*assertion].m_modified = 1;
}
}
//the real cache

void dcache_access( struct cache *c, unsigned addr, counter_t *saved, counter_t *miss_counter, counter_t *prefetch )
{
unsigned  index, tag;
index = (addr>>c->m_set_shift)&c->m_set_mask;
tag = (addr>>c->m_tag_shift);
...
if((c->m_tag_array[index+i*assertion].m_valid&&(c->m_tag_array[index+i*assertion].m_tag==tag))){
  c->m_tag_array[index+i*assertion].LRU = sim_num_insn;
  if (c->m_tag_array[index+i*assertion].m_modified==1){
    *saved = *saved+1;
    c->m_tag_array[index+i*assertion].m_modified= 0;
  }
  return;
}
...
assert( index+save*assertion < c->m_total_blocks );
if(change) {
dprefetch_cache_access( c, addr+32, prefetch);

*miss_counter = *miss_counter + 1;
c->m_tag_array[index+save*assertion].m_valid = 1;
c->m_tag_array[index+save*assertion].m_tag = tag;
c->m_tag_array[index+save*assertion].LRU = sim_num_insn;
c->m_tag_array[index+save*assertion].m_modified = 0;
}
...
sim_main(void)
{
  ...
  int dblock_size_bits = log2(32); //log2(blocksize) is the size of the beginning shift to ignore bytes
  int dtotal_blocks = (32*pow(2,10))/32; //32KB / 32B = 1024 blocks
  int dassociativity_bits = log2(4); //4-way has 2 bits which is log2(4)
  int dindex_bits = log2(dtotal_blocks)-dassociativity_bits; //bits of amount of blocks minus the bits for associativity for indexing
  struct cache *dcache = (struct cache *) calloc( sizeof(struct cache), 1 ); 
  dcache->m_total_blocks = dtotal_blocks;
  dcache->m_associativity = dassociativity_bits;
  dcache->m_set_shift = dblock_size_bits;
  dcache->m_set_mask = (1<<dindex_bits)-1;
  dcache->m_tag_shift = dindex_bits + dblock_size_bits;  
  dcache->m_tag_array = (struct block *) calloc( sizeof(struct block), dtotal_blocks );
  ...
  while (TRUE)
    {
      ...
      dcache_access(dcache, regs.regs_PC, &g_dcache_saved, &g_dcache_miss, &g_dcache_prefetch );     
