...
static counter_t g_total_taken_branches;
static counter_t g_total_load_stalls;
...
/* Frequencies */
  ...
  stat_reg_counter(sdb, "sim_taken_branches", 
  "total taken branch instructions executed",
  &g_total_taken_branches,
  0, NULL);

  stat_reg_counter(sdb, "sim_stalled_loads", 
  "total amount of stalls due to load",
  &g_total_load_stalls,
  0, NULL);
...
sim_main(void)
{
  ...
  //making a 2d array to hold instructions, [i][0] = opcode [i][1] = cycles since
  int i1, i2, i3, o1, o2;
  int instructions[5][4] = {0};
  int i;
  ...
  while (TRUE)
    {
      ...
      switch (op)
  //added in the i1 = I1... etc to save the register dependencies
	{
...
#include "machine.def"
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
	{
      ...
      for (i = 0; i < 4; i++){
        //move the instruction over (opcode)
        instructions[i][0] = instructions[i+1][0];
        //move the amount of cycles since it happened and add 1
        instructions[i][1] = instructions[i+1][1]+1;
        //move the destination registers for finding dependecies
        instructions[i][2] = instructions[i+1][2];
        instructions[i][3] = instructions[i+1][3];
      }
      if (MD_OP_FLAGS(op) & F_LOAD){
        //sets the "flag" to one to say that this is a load so might need to stall, also it wont be the cause of a stall
        instructions[4][0] = 1;
      }
      else{
        //doesnt need to stall cause not a load instruction (only loads and branches cause stalls and this
        // only needs to handle the load situations), and if the command is a store will never be a stall
        if ((MD_OP_FLAGS(op)& F_STORE)){
          instructions[4][0] =0;
        }
        else{
          instructions[4][0] =0;     
          //now to check for dependencies
          //i realized that holding more than two previous instructions in this array is uneeded
          for (i = 0; i < 5; i++){
            if (instructions[i][0]){
              if (i1 == instructions[i][2] || i1 == instructions[i][3]){
                if(instructions[i][1] < 2)
                  g_total_load_stalls++;
              }
              else if (i2 == instructions[i][2] || i2 == instructions[i][3]){
                if(instructions[i][1] < 2)
                  g_total_load_stalls++;
              }
              else if (i3 == instructions[i][2] || i3 == instructions[i][3]){
                if(instructions[i][1] < 2)
                  g_total_load_stalls++;
              }
            }
      ...
      //zero cycles since it happened
      instructions[4][1] = 0;
      //the registers that are being written to
      instructions[4][2] = o1;
      instructions[4][3] = o2;
      ...
      //actually not needed (+1 Instruction for every branch)
      int hold;

      hold = abs(regs.regs_NPC - regs.regs_PC);
      if (hold > sizeof(md_inst_t))
          g_total_taken_branches++; 
